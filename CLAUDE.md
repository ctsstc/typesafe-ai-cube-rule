...
A fun web app built on [cuberule.com](https://cuberule.com/): type any food and TypeSafe's Jev model classifies it by where its structural starch sits (toast, sandwich, taco, sushi, quiche, calzone, salad, cake, nachos). Hosted on Cloudflare Workers (static assets plus one API route) so it can be shared with friends.
...
- The API key (`TYPESAFE_API_KEY`) is server-side only. It must never reach the browser bundle, a commit, or a log line. Locally it lives in the root `.env`; in production it is a Worker secret (`wrangler secret put TYPESAFE_API_KEY`).
