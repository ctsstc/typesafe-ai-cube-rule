...
export function createClassifyHandler(
  ...
  return async function handleClassify(
    request: Request,
    env: Env,
    waitUntil: WaitUntil,
  ): Promise<Response> {
    try {
      return await classify(request, env, waitUntil, limiter);
    } catch (error) {
      console.error("classify: unhandled error", describe(error));
...
async function classify(
  ...
  const cacheKey = new Request(new URL(classifyUrl(item), url.origin));
  const kvKey = `v${QUESTION_SET_VERSION}:${item}`;
...
async function settle<T>(promise: Promise<T> | undefined): Promise<T | undefined> {
  try {
    ...
    console.warn("classify: storage read failed", describe(error));
    return undefined;
  }
}
...
function background(waitUntil: WaitUntil, label: string, promise: Promise<unknown> | undefined) {
  if (!promise) return;
  waitUntil(promise.catch((error) => console.warn(`classify: ${label} failed`, describe(error))));
}

function upstreamFailure(error: unknown): Response {
  const code = classifyError(error);
  console.error("classify: Jev call failed", {
    code,
    ...describe(error),
