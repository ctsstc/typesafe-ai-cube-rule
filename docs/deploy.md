...
It refuses to write when the KV writes would pass 900 minus today's `usage.calls` (see [D1 budget](#d1-budget)), when an eval ruling was fetched less than an hour ago or a backfill runs before 01:00 UTC (either would count as new foods in the last hour), or when the backfill's estimate passes `--max-usd`. It warns, without refusing, when its KV writes plus a full day of Jev calls at `DAILY_CALL_LIMIT` could pass 1,000.
...
That rule counts only the calls made so far. `DAILY_CALL_LIMIT` (1,000) never counts one-off writes, so on seed day it still allows enough Jev calls to take KV past 1,000 writes, and every ruling after that is billed but goes unstored, so other data centers pay Jev for it again. The 900 rule leaves about 100 writes of headroom for the rest of that UTC day, and `pnpm seed:docket` says so when the seed's KV writes plus a full day of Jev calls could pass 1,000. To rule it out, run the seed late in the UTC day, or deploy `DAILY_CALL_LIMIT` lowered by the seed's KV writes (814 for 186 eval rulings) and restore it after 00:00 UTC.
