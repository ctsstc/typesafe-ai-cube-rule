...
Between the gallery and How Jev rules. It makes the site feel lived in without showing anyone: foods that passed the listing checks, as the court heard them, never who asked and never a time. The lists started with the listed question set 7 eval rulings and foods visitors asked about before v1.3 (re-asked under question set 7, and single-ask v1.2 rows), so the section has something to show from day one.
...
| Honorary court | Not food. The court ruled anyway. | "definitely honorary salad", with the verdict's adverb (`VERDICT_ADVERBS`), or "officially honorary calzone" for a canon ruling. Honorary rulings only, Jev's most confident first |
| Friendship-ending | What people argue about most, by Jev's read. | "Debate: Friendship-ending" (`DEBATE_LABELS`) |

- **Hiding:** a list shows from a single entry (`MIN_LIST_ENTRIES` is 1) and hides only when it is empty. With every list empty, the lists off, or any error, the whole section renders nothing: no heading, no empty state. Jev vs the canon was dropped in v1.3: Jev agrees with all 45 canon rulings in the question set 7 eval, because most canon names are worked examples in its questions, so the card stayed empty.
- **Honorary** rulings are listed, with a small outlined "Honorary" marker after the name, except in Honorary court, whose line already says so.
...
- **Accessibility:** each list is an `<ol>` labelled by its h3. Link names read as a sentence ("Gyro: taco or sushi", "Canoe, Honorary: taco or sushi", "Good vibes: definitely honorary salad") through visually hidden punctuation. Every link is at least 44px tall. The section fades in over 600ms, and not at all with reduced motion.
...
- A food may show up in the public lists the first time anyone asks about it, if it passes `publicListing`: About names the checks as nothing abusive, no private people, no contact details and no gibberish. Pattern rules and Jev's reading screen out names of private people, phone numbers, email addresses, links and flagged text, and About says no screen catches everything. About also says the lists started with foods from our own testing (the seeded eval rulings) and foods people had already asked about, some from before the lists existed (the backfill, and v1.2 rows with one ask). D1's `rulings` table notes when each food was first asked. The lists never show who asked or a time.
...
- **v1.3 "Honorary court":** a food can be listed after one ask, every list shows from one entry, Honorary court replaces Jev vs the canon, and the lists start with the listed question set 7 eval rulings plus earlier visitors' foods re-asked under the current questions.
