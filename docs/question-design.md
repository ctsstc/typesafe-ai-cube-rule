...
- **What the Function adds on top** (not in `@cube/core`): an item is listed only after at least `MIN_ASKS` separate asks, a kill switch (the `PUBLIC_LISTS` var, or a D1 switch that `pnpm recent --lists off` flips) hides every list at once, the blocklist is checked every time the lists are read, and the activity line appears only above a threshold.
