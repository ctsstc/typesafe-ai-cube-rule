...
| `personal_info` | `hasPersonalInfo(item)`: a phone number (7 or more digits, counting digit words said in a row), an `@`, `http:`, `https:` or `www.`, a dot between letters such as `foo.com`, a spaced or ideographic dot before a common top-level domain, "dot com" spelled out, bracketed "[at]" or "(dot)", "x at y dot z", a webmail name, an underscore, a `$handle` or a platform name such as instagram or venmo |
| `private_person` | `person_kind` gives `private` a probability of `THRESHOLDS.publicPrivatePerson` (0.05) or more, or neither `none` nor `public` reaches `THRESHOLDS.publicPersonSure` (0.9). Jev cannot tell an invented full name from an obscure famous one, so an item must read clearly as nobody or a public figure |
...
- **Fails closed.** `listingScores(response)` turns a missing or NaN answer into `null`, and `listingBar` treats `null` as failing, so it hides the item.
- **Stored and rechecked.** The Function stores the four scores `listingBar` reads with each ruling, and `/api/lists` runs `listingBar` again on every row it serves. Tightening `publicPrivatePerson`, `publicPersonSure` or `publicAbusive` therefore hides stored rulings on the next refresh. Loosening one affects only rulings recorded afterwards, because a row hidden when it was recorded is never read again.
...
- **What the Function adds on top** (not in `@cube/core`): a kill switch (the `PUBLIC_LISTS` var, or a D1 switch that `pnpm recent --lists off` flips) hides every list at once, the blocklist is checked every time the lists are read, and the activity line appears only above a threshold. It adds no second ask: `MIN_ASKS` is 1, so the Jev call that first rules on an item lists it when `publicListing` passes.

`toListEntry(item, result)` turns `toCubeResult` output into the row a list shows, or `null` for a result that is never listed. Its `category` is **Jev's own pick** and `official` is the canon ruling, so a card shows `official ?? category`. Honorary court is the listed rows whose `kind` is `honorary`, most confident first. `confidence` is rounded to 3 decimals, `runnerUp` is the dissent (the runner-up with at least `THRESHOLDS.dissent`), and an honorary row has `wet: false` and `debateLevel: 0`.
...
| `publicPrivatePerson` | 0.05 | Public lists only. Invented full names inside food phrases scored as low as 0.06 ("tyler okonkwo sandwich"), so 0.15 listed two of them. 0.05 hides all 28 private people and "my coworkers" (0.06), a harmless loss |
| `publicPersonSure` | 0.9 | Public lists only. The second person bar: `none` or `public` must reach it. Every invented name that scored low on private read public between 0.63 and 0.86, so this bar hides all three on its own. It also hides "gordon ramsay's beef wellington" (public 0.88) and "shirley temple" (0.60). [docs/eval.md](eval.md#full-name-probes) has the sweeps |
